...
def end_coord_gridding3(X_init,Y_init,phi_init,L):
  ...
  return s
@njit(fastmath=False, cache=True)
def trajectory_planner5(end_coord_list,waypoint_array,X_init,Y_init,phi_init,L,Ts,N,vx):
  ...
  closest_midpoint1 = closest_point_list[1]
  closest_midpoint2 = closest_point_list[2]
  #closest_midpoint3 = closest_point_list[3]

  distance_left = np.linalg.norm(closest_midpoint2-end_coord_list[0])
  distance_right = np.linalg.norm(closest_midpoint2-end_coord_list[4])
  distance_mid = np.linalg.norm(closest_midpoint2-end_coord_list[2])
  delta_distance = distance_right - distance_left

  v1 = closest_midpoint2-closest_midpoint1
  v2 = end_coord_list[2]-end_coord_list[1]
  ...
  ###############
  steering_angle1 = math.acos(dot_product)
  steering_angle2 = math.atan(distance_mid/3)    # Change HERE PLS

  steering_angle = 0.5*steering_angle1+0.5*steering_angle2    # 0.785 and 0.125
  #vx = 5
  ...
  if delta_distance >0.0:
    ...
    steering_angle = -steering_angle
    """
  if cross_track_error_list[3] < 0.8:
    vx = 9
  else:
    vx = 5
...
def find_closest_point(p,array):
  ...
  closest_point = array[track_error_list.index(min(track_error_list))]
...
def turn_detection2(X_init,Y_init,phi_init,array,dis,num):
  array = array[:,0:2]
  ...
  p2 = p1 + vec_go
  p1_array = find_closest_point(p1,array)
  ...
  angle_sum = 0.
  for i in range(num):
    p2_array = find_closest_point(p2,array)
   ...
   # time_start = time.time()
    ...
    while not done:
       # time_now = time.time()
        ...
        vel = turn_detection2(obs['poses_x'][0],obs['poses_y'][0],obs['poses_theta'][0],waypoint_array,1.5,5)   #1.5
        print(vel)
        trajectory_chosen = np.array(trajectory_planner5(end_coord_list,waypoint_array,obs['poses_x'][0],obs['poses_y'][0],obs['poses_theta'][0],lf+lr,Ts,N,vel))
