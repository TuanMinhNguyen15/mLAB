...
def end_coord_gridding3(X_init,Y_init,phi_init,L):
  ...
  return s
@njit(fastmath=False, cache=True)
def trajectory_planner5(end_coord_list,waypoint_array,X_init,Y_init,phi_init,L,Ts,N):
  ...
  steering_angle = 0.8*steering_angle1+0.2*steering_angle2    # 0.785 and 0.125
  #vx = 5
   ...
   # time_start = time.time()
    ...
    while not done:
       # time_now = time.time()
        ...
        trajectory_chosen = np.array(trajectory_planner5(end_coord_list,waypoint_array,obs['poses_x'][0],obs['poses_y'][0],obs['poses_theta'][0],lf+lr,Ts,N))
